...
parser.add_argument("--random_ortho", type=int, default=4)
...
def prepare_depth_outputs():
    ...
    depth_map_node.inputs[1].default_value = 2
    depth_map_node.inputs[2].default_value = 4
...
def save_point_cloud(object_name, output_file):
...
def save_images(object_file: str) -> None:
    ...
    if os.path.exists(os.path.join(args.output_dir, object_uid, 'normal', '064.png')): return
...
#     bproc.init()
    ...
    #     save_point_cloud(object_name, os.path.join(args.output_dir, object_uid, 'point_cloud.ply'))
        ...
        if mat.use_nodes:
            ...
            for node in mat.node_tree.nodes:
                ...
                if node.type == 'TEX_IMAGE':
                    # 假设如果图像格式支持透明通道，则该纹理可能影响透明度
                    image = node.image
                    if image and image.file_format in ('PNG', 'TGA', 'BMP'):
                        # 检查该纹理是否连接到Principled BSDF节点的Alpha插槽
    ...
    for i in range(0, args.num_images, 4):
        ...
        render_and_save(i * (args.random_images+1), object_uid, -1, azimuth, elevation, distance, ortho=True)
        ...
        # render_and_save(i * (args.random_images+1), object_uid, len_val, azimuth, elevation, distance, ortho=False)
    ...
    #             filepath=os.path.join(args.output_dir, object_uid, ob.name + '.obj'),
